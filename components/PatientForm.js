...
export default function PatientForm() {
  ...
  if (submitted) {
    return (
      <div className="mx-auto flex max-w-md flex-col items-center gap-3 rounded-3xl border border-emerald-100 bg-white/90 p-8 text-center shadow-xl shadow-emerald-100 backdrop-blur dark:border-emerald-500/20 dark:bg-slate-900/70 dark:shadow-none">
        <div className="flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-emerald-400 to-emerald-600 text-white shadow-lg shadow-emerald-200 dark:shadow-none">
        ...
        <h2 className="text-lg font-semibold text-emerald-800 dark:text-emerald-300">Information submitted</h2>
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="mx-auto flex max-w-2xl flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-50">Patient Registration</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">Please fill in your information below.</p>
        </div>
        <span
      ...
      <div className="sticky top-16 z-10 rounded-2xl border border-white/60 bg-white/80 px-4 py-3 shadow-sm shadow-indigo-100 backdrop-blur dark:border-white/10 dark:bg-slate-900/70 dark:shadow-none">
      ...
      {FIELD_SECTIONS.map((section) => (
        <div
          key={section.title}
          className="flex flex-col gap-4 rounded-3xl border border-white/60 bg-white/80 p-5 shadow-lg shadow-indigo-100/70 backdrop-blur sm:p-6 dark:border-white/10 dark:bg-slate-900/60 dark:shadow-none"
        >
      ...
      <button
        type="submit"
        className="w-full rounded-full bg-gradient-to-r from-indigo-600 to-violet-600 px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-indigo-300 transition hover:shadow-xl active:opacity-90 sm:w-auto sm:self-end sm:px-8 dark:shadow-none"
      >
        Submit information
      </button>
    </form>
